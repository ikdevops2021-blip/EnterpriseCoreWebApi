-- ===================================================================================
-- DNAQMS API - BACKEND USERS & API KEYS PROVISIONING SCRIPT (MS SQL Server)
-- File Path: DNAQMSAPI/DatabaseScripts/MSSQLScript/102_ProvisionApiKeysAllUsers.sql
-- ===================================================================================
-- Description:
...
WHERE [Email] IN (N'admin@dnaqms.com', N'john@dnaqms.com') AND [IsDeleted] = 0;
...
DECLARE @AdminUserId INT = (SELECT TOP 1 Id FROM [User] WHERE [Email] = N'admin@dnaqms.com' AND [IsDeleted] = 0);

IF (@AdminUserId IS NOT NULL AND NOT EXISTS (SELECT 1 FROM [ApiKey] WHERE [UserId] = @AdminUserId AND [IsDeleted] = 0))
BEGIN
    INSERT INTO [ApiKey] (Id, KeyHash, Name, UserId, ExpiresAt, IsActive, CreatedBy, CreatedDate, ModifiedBy, ModifiedDate)
    VALUES (CAST(NEWID() AS CHAR(36)), N'rW3Z6P7H3m9Z5J9g8W5P1m9H3Z5J9g8W5P1m9H3Z5A=', N'System Admin Primary Integration Key', @AdminUserId, DATEADD(YEAR, 1, GETDATE()), 1, @AdminUserId, GETDATE(), @AdminUserId, GETDATE());
END
...
DECLARE @JohnUserId INT = (SELECT TOP 1 Id FROM [User] WHERE [Email] = N'john@dnaqms.com' AND [IsDeleted] = 0);

IF (@JohnUserId IS NOT NULL AND NOT EXISTS (SELECT 1 FROM [ApiKey] WHERE [UserId] = @JohnUserId AND [IsDeleted] = 0))
BEGIN
    INSERT INTO [ApiKey] (Id, KeyHash, Name, UserId, ExpiresAt, IsActive, CreatedBy, CreatedDate, ModifiedBy, ModifiedDate)
    VALUES (CAST(NEWID() AS CHAR(36)), N'kP3Z6P7H3m9Z5J9g8W5P1m9H3Z5J9g8W5P1m9H3Z5B=', N'John Doe Main HQ Integration Key', @JohnUserId, DATEADD(YEAR, 1, GETDATE()), 1, @JohnUserId, GETDATE(), @JohnUserId, GETDATE());
END
