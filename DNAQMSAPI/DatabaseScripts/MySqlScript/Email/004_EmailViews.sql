...
SELECT 
    ...
    CASE 
        ...
        THEN CAST((s.TotalSuccess / s.TotalProcessed) * 100 AS DECIMAL(5,2)) 
        ELSE 0 
    END AS SuccessRatePercentage,
    (SELECT ErrorDescription FROM EmailQueue 
     WHERE OrganizationId = s.OrganizationId 
       AND Status = 2 
       AND CAST(CreateDate AS DATE) = s.ReportDate
     ORDER BY CreateDate DESC LIMIT 1) AS LastErrorMessage
FROM MailStats s;
